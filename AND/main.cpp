...
void display(std::string name, DataSet &set, MoveMapper mapper) {
    std::cout << "Affichage " << name << std::endl;

    for (int i = 0; i < set.Inputs().size(); ++i) {
        std::cout << "-----Input-----" << std::endl;
        std::cout << "{";
        for (int j = 0; j < set.Inputs()[i].size(); ++j) {
            std::cout << mapper.getMove(set.Inputs()[i][j]);
            std::cout << ", ";
...
int main() {
    ...
    display("Train", set, mapper);

    DataSet set2;

    inData.clear();
    moves = parseMoves(
            "d4 d5 Bf4 c5 e3 cxd4 exd4 e6 Nf3 Nc6 c3 h6 Qa4 Bd7 Bb5 g5 Bg3 g4 Ne5 Nxe5 Bxe5 a6 Bxd7+ Qxd7 Qxd7+ Kxd7 Bxh8 Ne7");
    for (auto &move : moves) {
        float value = mapper.registerMove(move);
        inData.push_back(value);
    }
    ...
    inData.clear();
    moves = parseMoves(
            "d4 Nf6 Nf3 e6 e3 b6 Bd3 Bb7 c4 c5 Qc2 Bxf3 gxf3 cxd4 exd4 Nc6 d5 Nb4 Qc3 Rc8 a3 Nfxd5 Qb3 Nxd3+ Qxd3 Qc7 cxd5 Qxc1+ Qd1 Qxb2 dxe6 Rc1 exd7+ Kd8 Nd2 Rxa1 O-O Rxd1 Rxd1 Qc2");
    for (auto &move : moves) {
        float value = mapper.registerMove(move);
        inData.push_back(value);
    }
    ...
    inData.clear();
    moves = parseMoves(
            "d4 d5 c4 e6 Nc3 Bb4 Bd2 dxc4 e3 Bxc3 Bxc3 Qd5 Qa4+ b5 Qb4 Nc6 Qc5 a5 Qxd5 exd5 a3 Nge7 Nf3 Ba6 Be2 b4 axb4 axb4 Bd2 b3 O-O O-O Ne5 Nxe5 dxe5 g6 Bf3 c3 Bxc3 Bxf1 Rxa8 Rxa8 Kxf1 Ra1+ Ke2 c6 e4 dxe4 Bxe4 Ra4 f4 Rxe4+");
    for (auto &move : moves) {
        float value = mapper.registerMove(move);
        inData.push_back(value);
    }
    ...
    inData.clear();
    moves = parseMoves(
            "e3 e5 d4 Nc6 dxe5 Nxe5 Nf3 Bd6 Nxe5 Bxe5 Qf3 Qf6 Qd5 Ne7 Qd1 O-O h4 Bxb2 Bxb2 Qxb2 Nd2 Qb3 Nxb3");
    for (auto &move : moves) {
        float value = mapper.registerMove(move);
        inData.push_back(value);
    }
    ...
    display("Evaluation", set2, mapper);
